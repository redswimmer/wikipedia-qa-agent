# `wikipedia_answer_quality` eval dataset — design
...
evaluations/datasets/
  wikipedia_answer_quality.yaml            # 50 cases + 6 evaluators baked in
  wikipedia_answer_quality_schema.json     # auto-generated by Dataset.to_file()
```

No changes to `models.py` or `task.py` — this is the payoff of the existing
generic architecture; `uv run python -m evaluations.run wikipedia_answer_quality` works
...
- **Unit**: none needed for evaluator logic — all six are native
  ...
  new `test_wikipedia_answer_quality_dataset_loads`, following the existing
...
- **Live verification**: source all 50 cases into the committed YAML and
  ...
  wikipedia_answer_quality`) as the acceptance check during implementation —
...
- **README.md**: add `uv run python -m evaluations.run wikipedia_answer_quality` to
