...
No changes to `models.py`, `task.py`, or `run.py` — this is the payoff of
the existing generic architecture; `uv run python -m evaluations.run
hotpotqa_hard` works once the YAML exists, exactly as the original design
doc's "Scaling to more datasets" section anticipated.
